...
var jsToSygus = require('./parsers/js_parser')
var astCleaner = require('live-backend-nfn5/javascript/parsers/ast_cleaner.js')

//integrate the sygusSolution into the oldCode
var sygusToCode = function sygusToCode(sygusSolution, oldCode) {
  ...
  var newNewJsFxnBody = astToJs(astCleaner(smt_parser(newFxn)));

  var newCode;
  var fxns = extractFunctions(oldCode);
  for (i=0; i< fxns.length; i++){
    if (fxns[i].name == fxnName){
      newCode = [
        oldCode.slice(0, fxns[i].blockStart+2),
        "//"+oldCode.slice(fxns[i].blockStart+2, fxns[i].end-2).split("\n").join("\n//"),
        "\n"+newNewJsFxnBody+"\n}",
        oldCode.slice(fxns[i].end)].join('');
...
var extractFunctions = function extractFunctions(code) {
...
var codeToSygus = function codeToSygus(code, intendedExamples, currentEvalOfExamples) {
