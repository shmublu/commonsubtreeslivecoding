const vm = require('vm');

var sygus_converterFxns = require('./sygus_converter.js')
var codeToSygus = sygus_converterFxns.codeToSygus
var sygusToCode = sygus_converterFxns.sygusToCode
var eval_pbe_helpersFxns = require('../general/eval_pbe_helpers.js')
var updateCodeEval = eval_pbe_helpersFxns.updateCodeEval
var isPBEMode = eval_pbe_helpersFxns.isPBEMode

// when the user edits code, we keep tests up to date
var reeval = function reeval(code, examples) {
  //make a copy since js is pass-by-reference
  var newExamples = examples.slice();
  //console.log(code + " "+examples[0]);
  for (var i=0; i<newExamples.length; i+=1){
    try {
      var sandbox = {};
      vm.runInNewContext(code + " output=" + newExamples[i].call + ";", sandbox);
      var r = sandbox.output;
      newExamples[i].res = JSON.stringify(r);
      newExamples[i].resType = typeof(r);
    }
    catch (e) {
      console.warn("syntax error i think");
      // syntax error
      // pbe code is probably in process of being changed
    }
  };

  return newExamples;
};

function getTimeStamp(){
  var date = new Date();
  return "."+date.getTime();
}
// given code and examples,
// how can we fix the code to fit the examples
// The sygus file is written to path
var pbeRepair = function pbeRepair(code, examples, currentOutput, path) {
  //console.log("code: " + code + "examples: " + examples + "currentOutput: " + currentOutput + "path: " + path)
  const fs = require('fs');
  var sygusCode = codeToSygus(code, examples, currentOutput)
  fs.writeFileSync(path, sygusCode);
  fs.writeFileSync(path + getTimeStamp(), codeToSygus(code, examples, currentOutput));
  var execSync = require('child_process').execSync, cvc4Output;
  var cvc4Output = null;
  try {
    //changed from installation- doalarm timeout thing, sygus-stream on 11/17/20 (1->5)
    cvc4Output = execSync('doalarm () { perl -e \'alarm shift; exec @ARGV\' "$@"; }\n doalarm 55 cvc4 '+path).toString();
  }
  catch (e) {
    console.error("CVC4 call failed: "+e);
    //console.log("was an error message" + e + "but in summary CVC4 is not working for this set of examples")
    cvc4Output = null
  }
  if (cvc4Output !== null) {
    //console.log('begin sygusToCode(cvc4Output/sygusSolution, (old)code): ' + cvc4Output + ', ' + code)
    newCode = sygusToCode(cvc4Output, code);
  }
  else {
    newCode = code
  }
  
  return newCode;
}

var updateCodeEvalJS = function updateCodeEvalJS(code, examples, path) {
  return updateCodeEval({ repair: (c, ne, pe) => pbeRepair(c, ne, pe, path)
                        , eval:reeval }, code, examples)
}

module.exports = {
  reeval: reeval,
  pbeRepair: pbeRepair,
  updateCodeEvalJS: updateCodeEvalJS
}
